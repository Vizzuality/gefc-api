...
RSpec.describe WidgetsImporter, type: :model do
  ...
  it "sets Indicator.by_default for each indicator" do
    ...
    data_hash.each do |indicator_data|
      ...
      expect(Indicator.find_by(name: indicator_data["indicator_en"]).by_default).to eq(indicator_data["default"])
    end    
  end
  it "sets a relationship between Indicator and Widgets" do
    ...
    data_hash.each do |indicator_data|
      ...
      expect(Indicator.find_by(name: indicator_data["indicator_en"]).widgets.pluck(:name)).to eq(indicator_data["widget"])
    end    
  end
  it "sets subgroup_default for each Group" do
    ...
    data_hash.each do |indicator_data|
      if indicator_data["classification"]["subgroup_default"] == true
        ...
        expect(Group.find_by(name: indicator_data["classification"]["group_en"]).subgroup).to eq(Subgroup.find_by(name: indicator_data["classification"]["subgroup_en"]))
      else
        
        expect(Group.find_by(name: indicator_data["classification"]["group_en"]).subgroup).to_not eq(Subgroup.find_by(name: indicator_data["classification"]["subgroup_en"]))
      end
    end    
  end
end
