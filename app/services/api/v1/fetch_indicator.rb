  ...
  module V1
    class FetchIndicator
      ...
      def all
        ...
        Rails.cache.fetch('indicators') { Indicator.all.order(:name_en) }
      end

      def by_subgroup(subgroup)
        #Rails.logger.info("Fetching indicators by subgroup here!")
        Rails.cache.fetch([subgroup, 'indicators']) { subgroup.indicators.order(:name_en).to_a }
      end

      def default_by_subgroup(subgroup)
        #Rails.logger.info("Fetching indicator default_by_subgroup here!")
        Rails.cache.fetch([subgroup, 'default_indicator']) { subgroup.default_indicator }
      end
      ...
      def default_visualization(indicator)
        ...
        Rails.cache.fetch([indicator, 'default_visualization']) { default_widget(indicator).name }
      end

      def default_widget(indicator)
        #Rails.logger.info("Fetching indicator default_widget here!")
        Rails.cache.fetch([indicator, 'default_widget']) { indicator.default_widget }
      end

      def widgets_list(indicator)
        #Rails.logger.info("Fetching indicator widgets_list here!")
        Rails.cache.fetch([indicator, 'widgets_list']) { widgets(indicator).pluck(:name) }
      end

      def widgets(indicator)
        #Rails.logger.info("Fetching indicator widgets_list here!")
        Rails.cache.fetch([indicator, 'widgets']) { indicator.widgets }
      end

      def category_1(indicator)
        #Rails.logger.info("Fetching indicator category_1 here!")
        Rails.cache.fetch([indicator, 'category_1']) { records(indicator).pluck(Record.current_locale_column(:category_1)).uniq }
      end

      def category_filters(indicator)
        #Rails.logger.info("Fetching indicator category_filters here!")
        Rails.cache.fetch([indicator, 'category_1']) do
      ...
      def start_date(indicator)
        ...
        Rails.cache.fetch([indicator, 'start_date']) { records(indicator).where("year > ?", 1900).order(year: :asc).pluck(:year).first }
      end

      def end_date(indicator)
        #Rails.logger.info("Fetching indicator end_date here!")
        Rails.cache.fetch([indicator, 'end_date']) { records(indicator).where("year > ?", 1900).order(year: :desc).pluck(:year).first }
      end
      ...
      def records(indicator, params = {})
        if params.any?
          ...
          Rails.cache.fetch([indicator, 'records']) { indicator.records }
        end
      end

      def scenarios(indicator)
        #Rails.logger.info("Fetching indicator scenarios here!")
        Rails.cache.fetch([indicator, 'scenarios']) do
      ...
      def group(indicator)
        ...
        Rails.cache.fetch([indicator, 'group']) { indicator.group }
      end

      def subgroup(indicator)
        #Rails.logger.info("Fetching indicator subgroup here!")
        Rails.cache.fetch([indicator, 'subgroup']) { indicator.subgroup }
      end
      ...
      def by_id_or_slug(slug_or_id, filters, includes)
        ...
        Rails.cache.fetch(['indicator', slug_or_id]) do
      ...
      def regions(indicator)
        ...
        Rails.cache.fetch([indicator, 'regions']) { indicator.regions }
      end
    end
  end
end
