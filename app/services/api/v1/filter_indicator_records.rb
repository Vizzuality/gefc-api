  ...
  module V1
    class FilterIndicatorRecords
      ...
      def init_query(indicator, params)
        @query = if params["visualization"].nil?
          Record.where(indicator_id: indicator.id)
        else
          Widget.where(name: params["visualization"]).first.records.where(indicator_id: indicator.id)
        end
      end
